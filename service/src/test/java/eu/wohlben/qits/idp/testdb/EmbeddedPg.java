package eu.wohlben.qits.idp.testdb;
 ...
 * with no docker, and the store is postgres now — so Testcontainers and Quarkus dev services are
 * both out, and the schema still has to be exercised against the engine the service actually ships
 * on. Zonky resolves real postgres binaries as ordinary Maven artifacts and this class spawns them
 * as a child process: a dependency, not a daemon.
 *
 ...
 *
 * <p>Each suite that needs a store names its own database on that one instance — {@code idp_svc}
 * for the {@code @QuarkusTest} suite, {@code idp_packaged_it} for the packaged artifact — so the
 * two can never mean the same schema.
 */
